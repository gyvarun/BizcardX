...
# Comfiguring Streamlit dashboard
st.set_page_config(layout='wide')

# Title
st.title(':orange[Extracting Data, Empowering Connections]')
st.write(':red[Transforming Data into Insights for Smarter Decisions]')

# Tabs 

tab1, tab2, tab3 = st.tabs(["Data Extraction", "Data Updation", "Data Deletion"])

# ====== Data Extraction and upload zone  ====== #

with tab1:
    st.subheader(':green[**Upload Image**]')
    ...
    # ------ Display the processed card with green box ------ #

        col1, col2= st.columns(2)

        with col1:
            # Define a funtion to draw the box on image
            def draw_boxes(image, text_read, color='green', width=2):
    ...
    # --------------------------------------   /   Data Upload to Mysql   /   --------------------------------------- #
        ...
        Upload = st.button(':green[**Upload to MySQL DB**]', key='upload_button')
        ...
        st.info('Click on Browse files and upload an image')
